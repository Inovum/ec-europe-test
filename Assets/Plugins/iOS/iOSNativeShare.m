...
-(id) initWithText:(char*)text withURL:(char*)url withImage:(char*)image withSubject:(char*)subject{
	...
	if(mUrl != NULL && mUrl.length > 0){
        ...
        NSLog(@"!!! data mUrl=%@", mUrl);
        
        NSArray *strings = [mUrl componentsSeparatedByString:@"/"];
        NSLog(@"!!! data strings=%@", strings);
        NSString *fileName = [strings objectAtIndex:[strings count] - 1];
        NSLog(@"!!! data fileName=%@", fileName);
        
        NSString *documentDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) firstObject];
        NSString *filePath = [documentDir stringByAppendingPathComponent:fileName];
        
        //        NSURL *fileURL = [NSURL fileURLWithPath:mUrl];
        
        if(mSubject == NULL) {
            mSubject = fileName;
        }
        
        // instead of URL we will assume that URL is to the file on the filesystem of iOS device and share a document by that URL
        NSData *data = [NSData dataWithContentsOfFile:mUrl];
        [data writeToFile:filePath atomically:YES];
        NSLog(@"File %@ is re-saved to %@", mUrl, filePath);
